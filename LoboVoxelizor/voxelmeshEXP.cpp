...
VoxelMeshEXP::VoxelMeshEXP(TriangleMesh* p_mesh, int grid_dens, int method_)
    : VoxelMesh(p_mesh, grid_dens, method_)
{
}

//VoxelMeshEXP::~VoxelMeshEXP()
//{
//    cleanAll();
//}
...
// 2: SAT-based
//===============================================================
void VoxelMeshEXP::computeVoxelMeshSpeed(int method_idx)
{
...
void VoxelMeshEXP::markFaceBySATMethodSpeed(TriangleFace* facep)
{
...
void VoxelMeshEXP::markFaceByScanlineIntegerSpeed(TriangleFace* facep)
{
...
void VoxelMeshEXP::markFaceByScanlineSpeed(TriangleFace* facep)
{
    //Get the 3 vetex of the face.
    double p0[3], p1[3], p2[3];
    p0[0] = facep->node0->ori_coordinate[0];
...
int VoxelMeshEXP::mark3DLineSpeed(const double start_point[], const double end_point[],
...
void VoxelMeshEXP::mark3DLineSpeed(const double start_point[], const double end_point[],
                                   const int start_point_vox[], const int end_point_vox[])
{
    //If start_point and end_point are in the same vox, simply mark start_point then return.
//    if (start_point_vox[0] == end_point_vox[0] &&
...
void VoxelMeshEXP::mark2DLineSpeed(const double start_x, const double start_y,
...
//===============================================================
// For detail process of voxelization.
//===============================================================
void VoxelMeshEXP::markFaceByScanlineIntegerDetailed(TriangleFace* facep, int marker)
{
    //Get the 3 vetex of the face.
    double p0[3], p1[3], p2[3];
    p0[0] = facep->node0->ori_coordinate[0];
    ...
    total_triangle_area += computeTriangleArea(p0,p1,p2);
    //---------------------------------------------------------------
    ...
    //record the three vetices for animation.
    vertex_list.resize(3);
    vertex_list[0] = p0_vox[0] + (p0_vox[1] + p0_vox[2]*grid_den) * (long long)grid_den;
    vertex_list[1] = p1_vox[0] + (p1_vox[1] + p1_vox[2]*grid_den) * (long long)grid_den;
    vertex_list[2] = p2_vox[0] + (p2_vox[1] + p2_vox[2]*grid_den) * (long long)grid_den;

    // If the case is simple, no complicate computation is needed.
    if (p0_vox[0] == p1_vox[0] && p0_vox[1] == p1_vox[1] && p0_vox[2] == p1_vox[2] &&
        p0_vox[0] == p2_vox[0] && p0_vox[1] == p2_vox[1] && p0_vox[2] == p2_vox[2])
    {
        markPoint(p0_vox[0] + (p0_vox[1] + p0_vox[2]*grid_den) * (long long)grid_den, marker);
        //voxel_map[p0_vox[0] + p0_vox[1]*grid_den + p0_vox[2]*grid_den*(long long)grid_den] = 1;
        return;
    }
    ...
    //record the sequence.
    edge_list.resize(3);
    edge_list[0] = new vector<long long>; // record p0 ~ p2
    edge_list[1] = new vector<long long>; // record p0 ~ p1
    edge_list[2] = new vector<long long>; // record p1 ~ p2

    for (int k=0; k<size_1; k++)
    {
        int* vox = voxsequ_1[k];
        edge_list[0]->push_back(vox[0] + (vox[1] + vox[2]*grid_den) * (long long)grid_den);
    }
    for (int k=0; k<size_2; k++)
    {
        int* vox = voxsequ_2[k];
        edge_list[1]->push_back(vox[0] + (vox[1] + vox[2]*grid_den) * (long long)grid_den);
    }
...
//                                  axis_x, axis_y, axis_z, slice_i, marker);
            ...
            //record the sequence.
            vector<long long>* p_scanline = new vector<long long>;
            p_scanline->clear();
            scanline_list.push_back(p_scanline);
            p_scanline->push_back(slice_i); // the first element in scanline indicate the idx of the slice.
            ...
            while (!(i_x == length_x && i_y == length_y))
            {
                ...
                //record the voxel.
                p_scanline->push_back(moving_vox_idx);
    ...
    {
        int* vox = voxsequ_2[k];
        edge_list[2]->push_back(vox[0] + (vox[1] + vox[2]*grid_den) * (long long)grid_den);
    }
...
void VoxelMeshEXP::computeVoxelMesh()
{
    // return if no triangle mesh or invalid grid density.
    if (!triangle_mesh)
    {
    ...
    // Clear the old node/element lists.
    cleanAll();
    ...
    //The voxel map is a unsigned char array. One byte for one voxel.
    long long sizearray = grid_density * grid_density *(long long)grid_density;
    long long voxel_map_size = (sizearray>>2) + 1;
...
void VoxelMeshEXP::voxelizeTriangleMesh()
{
    cout << "This is EXP voxelizing triangle mesh.\n";

    time_surface = 0.0;
    time_interior = 0.0;
    time_incremental_part = 0.0;
    ...
    {
        // Use float scanline method to voxelize the surface of the sub mesh.
        QElapsedTimer timer;
        timer.start();
        for (int i=0; i<triangle_mesh->num_mesh; ++i)
        {
            ...
            {
                markFaceByScanlineIntegerDetailed(*fi, 1);
            }
        }
        time_surface = timer.nsecsElapsed();
    }
    ...
    {
        // Use integer scanline method to voxelize the surface of the sub mesh.
        QElapsedTimer timer;
        timer.start();
        for (int i=0; i<triangle_mesh->num_mesh; ++i)
        {
    ...
    time_surface = time_surface * 0.001;

    // statistic result.
    //cout << "The time of incremental part was " << time_incremental_part * 0.01 * 0.001 << endl;
    //cout << "Average area of triangle = " << total_triangle_area/triangle_mesh->num_face << endl;
    //cout << "Ratio of area to dimension = " << total_triangle_area/triangle_mesh->num_face/voxel_dim << endl;
    cout << "prim cmp add mul = " << op_op << " " << op_cmp << " " << op_add << " " << op_mul << endl;
    ...
    // Only one merged element mesh is generated.
    generateELEMesh();
    ...
    {
        int size = mesh_list[i]->size();
        for (int j=0; j<size; ++j)
        {
            if (mesh_list[i]->at(j)->isOnSurfaceVoxelization)
            {
                num_ele_surface++;
            }
            else
...
void VoxelMeshEXP::markFaceBySATMethod(TriangleFace* facep, int marker)
{
    //Get the 3 vertices of the face.
    double v[3][3];
    v[0][0] = facep->node0->ori_coordinate[0];
        ...
        for (int yi = vox_min[1]; yi <= vox_max[1]; ++yi)
        {
            ...
            for (int zi = vox_min[2]; zi <= vox_max[2]; ++zi)
            {
                ...
                if ((np + d1) * (np + d2) <= 0.0)
                {
                    ...
                    {
                        //overlap, mark the voxel.
                        long long vox_idx = xi + yi*grid_density +
                                            zi*grid_density*(long long)grid_density;
...
void VoxelMeshEXP::markFaceByScanlineInteger(TriangleFace* facep, int marker)
{
    //Get the 3 vetex of the face.
    double p0[3], p1[3], p2[3];
    p0[0] = facep->node0->ori_coordinate[0];
    ...
    int voxsequ_1[LONGEST_EDGE][3];
    int voxsequ_2[LONGEST_EDGE][3];
    int size_1 = mark3DLine(p11, p0, p11_vox, p0_vox, voxsequ_1, marker);
    int size_2 = mark3DLine(p1, p0, p1_vox, p0_vox, voxsequ_2, marker);
...
//                                  axis_x, axis_y, axis_z, slice_i, marker);
            ...
            long long moving_vox_idx = vox_1[0] + vox_1[1]*grid_den +
                                       vox_1[2]*grid_den*(long long)grid_den;
...
void VoxelMeshEXP::markFaceByScanline1D(TriangleFace* facep, int marker)
{
    //Get the 3 vetex of the face.
    double p0[3], p1[3], p2[3];
    p0[0] = facep->node0->ori_coordinate[0];
...
//cout << "slice " << slice_idx << " " << endl;
    ...
    // voxelize the last slice.
    markSliceTriangle(startp0_x, startp0_y, startp1_x, startp1_y, p2[axis_x], p2[axis_y],
                      axis_x, axis_y, axis_z, slice_idx, marker);
}

void VoxelMeshEXP::markFaceByScanline(TriangleFace* facep, int marker)
{
    //Get the 3 vetex of the face.
    double p0[3], p1[3], p2[3];
    p0[0] = facep->node0->ori_coordinate[0];
...
//    }
    ...
    while  (plane < p1[axis_z])
    {
        ...
        while (length > 0.0)
        {
            mark2DLine(startp0_x, startp0_y, startp1_x, startp1_y,
                       axis_x, axis_y, axis_z, slice_idx, marker);
    ...
    mark2DLine(endp0_x, endp0_y, endp1_x, endp1_y,
               axis_x, axis_y, axis_z, slice_idx, marker);
...
void VoxelMeshEXP::mark2DLine(const double start_x, const double start_y,
                              const double end_x, const double end_y,
...
void VoxelMeshEXP::mark2DLineInteger(const int start_pixel_x, const int start_pixel_y, const int end_pixel_x, const int end_pixel_y,
                                     int axis_x, int axis_y, int axis_z, int slice_idx, int marker)
{
    // Compute the step distance parameters.
...
inline void VoxelMeshEXP::computeVoxIdx(int voxidx[], const double pp[])
{
...
inline int VoxelMeshEXP::computeVoxOneIdx(double coor, int axis_idx)
{
...
inline int VoxelMeshEXP::computeDomiAxis(const double pp0[3], const double pp1[3], const double pp2[3])
{
    // compute the norm, i.e. (p1p0)x(p2p0).
    double v1[3], v2[3];
    v1[0] = pp1[0] - pp0[0];
...
double VoxelMeshEXP::computeTriangleArea(double pp0[], double pp1[], double pp2[])
{
...
inline void VoxelMeshEXP::sortVertByDomiAxis(double pp0[], double pp1[], double pp2[], int dd)
{
    double temp[3];
...
inline void VoxelMeshEXP::assignPoint(double pp0[], const double pp1[])
{
    pp0[0] = pp1[0];
    pp0[1] = pp1[1];
    pp0[2] = pp1[2];
...
inline void VoxelMeshEXP::markPoint(const long long vox_idx, int marker)
{
...
void VoxelMeshEXP::recordPoint(const int xx, const int yy, const int zz, vector<long long>* p_scan)
{
